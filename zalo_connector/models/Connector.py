...
class AcruxChatConnector(models.Model):
    ...
    source = fields.Char('Account (Instagram-FaceBook-Zalo)', required=True, default='/',
                         help='Instagram, FaceBook or Zalo phone number.')
    ...
    auto_valid_number = fields.Boolean('Validate Numbers', default=False,
                                       help='Check if it exists in Zalo and repair.')
    ...
    def compute_webhook_url(self):
        for record in self:
            if record.odoo_url and record.uuid:
                record.webhook_url = '%s/acrux_webhook/zalo_connector/%s' % \
    ...
    def check_is_valid_old_records_ids(self, check_conv_ids):
        ...
        if self.connector_type != 'apichat.io':
            raise UserError(_('Available only for Zalo connector through apichat.io.'))
    ...
    def check_is_valid_zalo_number(self, conv_ids, overwrite=True, raise_error=True):
    ...
    def ca_get_check_number(self, list_numbers, raise_error=True):
        ...
        try:
            datas = conn_id.ca_request('zalo_number_get', params=params, timeout=30)
            ...
            if raise_error:
                ...
                return {'error': error}
        for n in numbers:
            res[n['id']] = {'valid': bool(n['zalo_id'] or ''),
                            'same': bool(n['id'] == n['zalo_id']),
                            'number': n['zalo_id']}
    ...
    def ca_get_status(self):
        ...
        if acrux_ok:
            ...
            if accountStatus == 'authenticated':
                ...
                self.message = False
                message = 'All good!'
                detail = 'Zalo connects to your phone to sync messages. ' \
