...
class AcruxChatConversation(models.Model):
    ...
    number = fields.Char('Base number')
    ...
    valid_number = fields.Selection([('yes', 'Yes'),
                                     ('no', 'No')], string='Valid', default=False, help='Exists in Zalo')
    ...
    def create(self, vals_list):
        ...
        for ret in res:
            if (self.env.context.get('is_from_wizard') or self.env.context.get('is_acrux_chat_room')) \
                ...
                ret.connector_id.check_is_valid_zalo_number(ret, raise_error=False)
                if ret.valid_number == 'no':
                    error = _('Number not exist in Zalo (%s)') % ret.number
    ...
    def action_check_is_valid(self):
        ...
        for conn in recs.values():
            ...
            conn['conn_id'].check_is_valid_zalo_number(conn['conv_ids'])
    ...
    def new_message(self, data):
        data['connector_id'] = self.env['acrux.chat.connector'].search([], limit=1).id
        data['res_partner_id'] = data['sender']
        partner = self.env['res.partner'].browse(data['sender'])
        data['name'] = data['number'] = partner.phone if partner.phone else ''
        conversation = self.search([('res_partner_id', '=', partner.id)], limit=1)
        if not conversation:
        ...
        message_id = self.env['acrux.chat.message'].create({
            'user_id': conversation.agent_id.id,
            'text': data['message'],
    ...
    def send_message_product(self, prod_id):
        ...
        attach = self.env['ir.attachment'].search([('res_model','=','product.product'),('res_id','=',product_id.id)], limit=1)
        if attach:
            list_price = formatLang(self.env, product_id.list_price, currency_obj=self.env.user.company_id.currency_id)
            caption = '%s\n%s / %s' % (product_id.name.strip(), list_price, product_id.uom_id.name)
            msg_data = {
                'text': caption,
                'from_me': True,
                'is_product': True,
    ...
    def check_object_reference(self, postfix, view):
        return self.sudo().env['ir.model.data'].check_object_reference('zalo_connector%s' % (postfix or ''), view)
