...
class ZaloMessageList(models.Model):
    ...
    def copy(self, default=None):
        default = default or {}
        chat_list = super(ZaloMessageList, self).copy(default)
...
class ZaloMessageListItem(models.Model):
    ...
    def copy(self, default=None):
        default = default or {}
        item = super(ZaloMessageListItem, self).copy(default)
...
class ZaloMessageListRelation(models.AbstractModel):
    _name = 'acrux.chat.message.list.relation'
    _description = 'List Relation Class'

    chat_list_id = fields.Many2one('acrux.chat.message.list', string='Zalo List', ondelete='set null')

    def unlink(self):
        list_to_del = self.mapped('chat_list_id')
        out = super(ZaloMessageListRelation, self).unlink()
